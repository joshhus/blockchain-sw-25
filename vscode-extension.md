...
<area href="/docs/blockchain-sw-25?topic=blockchain-sw-25-develop-vscode#develop-vscode-creating-a-project" alt="Create a smart contract project" title="Create a Smart contract project" shape="rect" coords="28, 70, 153, 130" />
...
<img src=\"images/2-x_Pill.png\" alt=\"version 2.x\" width=\"30\" style=\"width:30px; border-style: none\"/> To install v2.0 of the extension:
...
<img src=\"images/1-4_Pill.png\" alt=\"version 1.4\" width=\"30\" style=\"width:30px; border-style: none\"/> To install v1.4 of the extension:

1. Go to the [GitHub releases page](https://github.com/IBM-Blockchain/blockchain-vscode-extension/releases){: external}
2. Select the most recent v1 release of the extension (tagged 1.0.x) and download the `.vsix` file.
...
4. **If you selected JavaScript, TypeScript, or Java**, s elect an asset to be managed by the example contract. For example, ***bond***.
...
You can use the VS Code extension to deploy your smart contract to a preconfigured Hyperledger Fabric network that the extension creates on your local machine. You can then deploy and test your smart contract before you deploy it to a live network.
...
### Deploying your smart contract with Fabric v2.x
{: #develop-vscode-deploy-smart-contract-v2}

After [upgrading the extension](https://code.visualstudio.com/docs/editor/extension-gallery#_extension-autoupdate){: extension} to use Fabric v2.x, smart contracts can be deployed using the new **deploy view**. This view replaces the install, instantiate, upgrade flow of deploying smart contracts that was used in the Fabric v1.x chaincode lifecycle. Instead, the new deployment steps (install, approve, commit) are all handled by the "deploy" view.

If you are using an earlier version of the VS Code extension with a `.cds` smart contract, you cannot use these steps. Refer to [Deploying your smart contract to the preconfigured network](#develop-vscode-deploy-smart-contract) instead.
...
3. (Optional) Use the **Additional peers to endorse commit transactions** dropdown to select or deselect any available additional peers you want to endorse the commit of this smart contract definition.


If you have changed your smart contract code and then repackaged it, you can use the deploy view and follow the same steps as above to upgrade the smart contract definition that you have deployed to the network. Follow these steps if you are using an earlier version of the VS Code extension and a `.cds` smart contract package.

### Interacting with your smart contract
{: #develop-vscode-submitting-transactions}

After a smart contract is  deployed, you can submit transactions to the functions inside your smart contract by using the **Fabric Gateways** pane:

1. Ensure that your smart contract is deployed, and that you are connected to the network.
2. In the **Fabric Gateways** pane, expand the **Channels** dropdown. Click the channel that the smart contract is deployed on.
...
4. Select **Debug Smart Contract configuration** from the drop-down list in the upper left.5. Click the **play** button. The extension will automatically package and deploy the smart contract on the local network.
...
## Step six: Test  a deployed smart contract{: #develop-vscode-testing-deployed-smart-contract}


You can generate tests for smart contracts that are committed on the networks that you connect to. The tests can be generated as either **JavaScript** or **TypeScript** for Node projects, **Java** for Java projects, or **Go** for Go projects.
...
### Invoke a smart contract that has been deployed on your channels
{: #develop-vscode-connecting-ibp-invoke}

You can download your connection profile from the {{site.data.keyword.blockchainfull_notm}} Platform console to build a gateway in the **Fabric Gateways** pane. You can then use the gateway to invoke the smart contracts that were installed and instantiated deployedon your network.
